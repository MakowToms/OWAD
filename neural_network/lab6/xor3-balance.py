...
# load dataset
train, test = load_data(file_name='xor3-balance', folder='classification', classification=True)
...
plt.subplot(121)
plot_data_2d(x[:, 0], x[:, 1], y[:, 0], title='True classes of points from xor3 balance dataset', show=False)
...
plt.subplot(122)
plot_measure_results_data(measures, labels=labels, title_base="accuracy", ylabel="accuracy", title_ending=" for different regularization methods", from_error=1)
plt.show()
