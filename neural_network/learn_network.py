...
def learn_network(x, y, hidden_layers, activations, initialize_weights='norm', momentum_type='RMSProp', lambda_momentum=0.9, beta=0.01, eta=0.01, epochs=1, batch_size=32, iterations=20, regularization_lambda=0, regression=True, plot_title=None, return_result=False, plot=True, plot_show=True, ):
    ...
    for i in range(iterations):
        ...
        if momentum_type == 'normal':
            network.backward(x, y, eta=eta, epochs=epochs, batch_size=batch_size, regularization_lambda=regularization_lambda)
        if momentum_type == 'momentum':
            network.backward(x, y, eta=eta, epochs=epochs, batch_size=batch_size, regularization_lambda=regularization_lambda, lambda_momentum=lambda_momentum, moment_type='momentum')
        if momentum_type == 'RMSProp':
            network.backward(x, y, eta=eta, epochs=epochs, batch_size=batch_size, regularization_lambda=regularization_lambda, beta=beta, moment_type='RMSProp')
