...
def plot_measure_results_data(errors, title_base='MSE', title_ending=' through epochs', labels=["base model", "momentum model", "RMSProp model"], colors=['red', 'green', 'cyan', 'yellow'], xlabel='epochs', ylabel='MSE', from_error=0, show=True):
    n = len(errors[0])
...
def plot_data_1d_compare(x, true, predictions, title='Fictional data with values', ylabel='result values', xlabel='observed values', labels=["true data", "prediction"], show=False):
    plt.plot(x[:, 0], true[:, 0], 'bo')
    different_colors = ['r', 'g', 'c', 'y', 'k']
